...
Last year, our community developed a new metadata schema for GeoBlacklight, lovingly called _Aardvark_. This new schema is a significant change: it emphasizes interoperability and the unique characteristics of geodata & maps, so we decided to begin promoting it as an all-purpose discovery schema for geospatial resources. To that end, we launched a new website, [opengeometadata.org](https://opengeometadata.org) that includes detailed schema documentation, guides, and examples.
...
_Image Caption - Two bounding boxes_
...
_Image Caption - The map is now directly below the title and has a full-screen option. There is also a new button for web services and an adjusted dropdown button for Downloads_

![index-map-color](blog-images/index-map-color.png)
_Image Caption - Shows improved colors and a legend above the index map_
